...
base = Path(r"C:\masterresearch\Comparative_advantage")
gaez_dir = base / "GAEZ"
glofas_dir = base / "GloFAS"
script_path = gaez_dir / "glofas_reliable_river_pipeline.py"
notebook_path = gaez_dir / "glofas_reliable_river_download.ipynb"
    ...
    "cells": [
        {
            ...
            "metadata": {},
            "source": [
                "# GloFAS reliable river download and processing\n",
                "\n",
                "GloFAS historical の日次 river discharge から、灌漑可能性 proxy として `reliable_river` と `distance_to_reliable_river` を作るノートです。\n",
                "\n",
                "デフォルトは 2020 年だけのテスト実行です。動作確認後、`glofas_reliable_river_pipeline.py` の `YEARS` を `list(range(1991, 2021))` に変えて本番実行します。\n"
            ],
        },
        {
            "cell_type": "markdown",
            ...
            "source": [
                "## 事前設定\n",
                "\n",
                "EWDS/Copernicus にログインし、`cems-glofas-historical` の CEMS-FLOODS licence を承認した上で、`C:\\Users\\tsuda\\.cdsapirc` を作成します。\n",
                "\n",
                "```text\n",
                "url: https://ewds.climate.copernicus.eu/api\n",
                "key: <YOUR_PERSONAL_ACCESS_TOKEN>\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "print('cdsapi installed:', importlib.util.find_spec('cdsapi') is not None)\n",
                "print('.cdsapirc exists:', (Path.home() / '.cdsapirc').exists())\n",
            ],
        },
        {
            "cell_type": "code",
            ...
            "outputs": [],
            "source": [
                "%run \"C:/masterresearch/Comparative_advantage/GAEZ/glofas_reliable_river_pipeline.py\"\n",
            ],
        },
    ],
    "metadata": {
...
for directory in [glofas_dir, glofas_dir / "raw_zip", glofas_dir / "raw_netcdf", glofas_dir / "annual_p10_source_0p05", glofas_dir / "processed_5min", glofas_dir / "logs"]:
    directory.mkdir(parents=True, exist_ok=True)

print(f"Wrote script: {script_path}")
print(f"Wrote notebook: {notebook_path}")
print(f"Prepared data directory: {glofas_dir}")
